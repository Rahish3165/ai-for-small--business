...
export default function Dashboard({ onBack }: DashboardProps) {
  ...
  const DEMO_BUSINESS = { name: "Ramesh Kirana & Tea Stall", owner_name: "Ramesh Kumar" };
  const DEMO_TRANSACTIONS = [
    { id: '1', created_at: new Date(Date.now() - 1000 * 60 * 35).toISOString(), description: 'UPI QR Payment - Evening Tea & Snacks', category: 'Sales', type: 'INCOME', amount: 4250 },
    { id: '2', created_at: new Date(Date.now() - 1000 * 60 * 120).toISOString(), description: 'Amul Taaza Milk Crates (20L)', category: 'Dairy Supplies', type: 'EXPENSE', amount: 1200 },
    { id: '3', created_at: new Date(Date.now() - 1000 * 60 * 360).toISOString(), description: 'Paytm Soundbox - Bun Maska & Chai', category: 'Sales', type: 'INCOME', amount: 3800 },
    { id: '4', created_at: new Date(Date.now() - 1000 * 60 * 600).toISOString(), description: 'Wholesale Sugar (20kg) & Chai Masala', category: 'Raw Materials', type: 'EXPENSE', amount: 950 },
    { id: '5', created_at: new Date(Date.now() - 1000 * 60 * 1440).toISOString(), description: 'Daily UPI Collection (Counter)', category: 'Sales', type: 'INCOME', amount: 5120 },
    { id: '6', created_at: new Date(Date.now() - 1000 * 60 * 1800).toISOString(), description: 'Disposable Paper Cups (500 pcs)', category: 'Packaging', type: 'EXPENSE', amount: 450 },
    { id: '7', created_at: new Date(Date.now() - 1000 * 60 * 2880).toISOString(), description: 'Weekend Rush Sales', category: 'Sales', type: 'INCOME', amount: 6400 },
    { id: '8', created_at: new Date(Date.now() - 1000 * 60 * 3200).toISOString(), description: 'Commercial Gas Cylinder Refill', category: 'Utilities', type: 'EXPENSE', amount: 1100 }
  ];
  const DEMO_INVENTORY = [
    { id: '1', item_name: 'Fresh Cow Milk (Amul)', quantity: 4, unit: 'Liters', threshold: 10 },
    { id: '2', item_name: 'Special Chai Leaves (Wagh Bakri)', quantity: 12, unit: 'Kg', threshold: 5 },
    { id: '3', item_name: 'Refined Sugar', quantity: 3, unit: 'Kg', threshold: 8 },
    { id: '4', item_name: 'Cardamom & Fresh Ginger', quantity: 2, unit: 'Kg', threshold: 1 },
    { id: '5', item_name: 'Fresh Bun & Rusks', quantity: 28, unit: 'Packets', threshold: 15 },
    { id: '6', item_name: 'Paper Cups (150ml)', quantity: 45, unit: 'Pcs', threshold: 100 }
  ];

  // Fetch initial context for AI and KPIs
  useEffect(() => {
    const applyContextData = (biz: any, txns: any[], invt: any[]) => {
      ...
      // Generate smart suggestions from real or demo data
    ...
    const fetchContext = async () => {
      try {
        ...
        const biz = (businesses && businesses.length > 0) ? businesses[0] : DEMO_BUSINESS;
        const txns = (transactions && transactions.length > 0) ? transactions : DEMO_TRANSACTIONS;
        const invt = (inventory && inventory.length > 0) ? inventory : DEMO_INVENTORY;
        applyContextData(biz, txns, invt);
      } catch (e) {
        console.warn("Context fetch failed, falling back to demo data:", e);
        applyContextData(DEMO_BUSINESS, DEMO_TRANSACTIONS, DEMO_INVENTORY);
      }
    };
    fetchContext();
  }, []);
